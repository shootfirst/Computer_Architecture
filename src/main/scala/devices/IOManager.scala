...
//  printf(p"[IO] Serial: ${io.serial}\n")
  ...
  val ifWait_reg  = RegInit(waitNone)
  val memWait_reg = RegInit(waitNone)
  ...
  handleOutput(ifWait_reg, if_)
  handleOutput(memWait_reg, mem)

  // Status after output
  val flashFree = ifWait_reg =/= waitFlash && memWait_reg =/= waitFlash
  ifWait_reg := Mux(ifWait_reg =/= waitNone && !if_.ok, ifWait_reg, waitNone)
  memWait_reg := Mux(memWait_reg =/= waitNone && !mem.ok, memWait_reg, waitNone)

  // Handle input. MEM first.
  when(memWait_reg === waitNone && mem.mode =/= RAMMode.NOP) {
    when(mem.addr.atRAM) {
      bindInput(mem, io.ram)
      when(RAMMode.isWrite(mem.mode)) {
        memWait_reg := waitNone
        mem.ok := true.B
      }.otherwise {
        memWait_reg := waitRAM
        mem.ok := false.B
...
//      stop(1)
  ...
  when(ifWait_reg === waitNone && memWait_reg === waitNone && mem.mode === RAMMode.NOP && if_.mode =/= RAMMode.NOP) {
    when(if_.addr.atRAM) {
      bindInput(if_, io.ram)
      ifWait_reg := waitRAM     // Readonly, always wait
      if_.ok := false.B
    }.elsewhen(if_.addr.atFlash) {
      bindInput(if_, io.flash)
      ifWait_reg := waitFlash
      if_.ok := false.B
    }.otherwise {
